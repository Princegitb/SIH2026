...
export default function AlertsView() {
  ...
  if (loading || !data) {
    return (
      <div className="flex items-center justify-center h-[50vh]">
        ...
        <span className="ml-3 text-slate-500">Scanning sensitive receptor boundaries...</span>
      </div>
    )
  }
  ...
  return (
    <div className="space-y-6">
      ...
      {alerts.length === 0 ? (
        <div className="glass-panel rounded-xl p-8 flex flex-col items-center justify-center space-y-3 h-[250px] text-center border-emerald-200 bg-emerald-50/50">
          <ShieldCheck className="text-emerald-600" size={48} />
          <div className="text-sm font-bold text-slate-800">System Status: Secure</div>
          <p className="text-xs text-slate-600 max-w-sm">No active Formaldehyde columns or active fire fronts detected near schools or hospitals in {selectedDistrict} today.</p>
        </div>
      ) : (
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          {alerts.map((alert, idx) => (
            <div key={idx} className="glass-panel rounded-xl p-5 flex items-start space-x-4 border-red-200 bg-red-50/50">
              ...
              <div className="flex-1 space-y-2">
                <div className="flex justify-between items-start">
                  <div>
                    ...
                    <span className="text-[9px] bg-red-100 text-red-600 border border-red-200 font-bold px-1.5 py-0.5 rounded uppercase tracking-wider">
                      {alert.type} Proximity Warn
                    </span>
                  </div>
                  <span className="text-xs font-bold text-red-500">{alert.dist_km} km away</span>
                </div>
                ...
                <div className="text-[10px] text-red-700 font-bold bg-red-50 border border-red-100 rounded p-2 flex items-center space-x-1.5">
