  ...
  return (
    <div className="space-y-6">
        ...
        <div className="flex flex-col lg:flex-row justify-between items-start lg:items-center gap-5">
          <div>
            <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-purple-500/20 text-purple-300 text-xs font-bold border border-purple-500/30 mb-2">
              <Satellite size={13} className="text-purple-300 animate-pulse" />
            ...
            <p className="text-sm text-slate-300 max-w-3xl mt-1 leading-relaxed">
              Real-time intervention modeling for <strong className="text-purple-300">{district} ({targetSummary?.state || 'Punjab'})</strong>: 
              Simulates local policy decisions based on actual <span className="text-amber-300 font-semibold">NASA VIIRS fires</span>, <span className="text-blue-300 font-semibold">Sentinel-5P gas columns</span>, and <span className="text-emerald-300 font-semibold">Chemical Mass Balance source apportionment</span>.
            </p>
          </div>
      ...
      {simData?.satellite_reasoning && (
        <div className="glass-panel p-5 rounded-3xl border border-purple-500/40 bg-purple-950/20 shadow-lg space-y-3">
          <div className="flex flex-wrap items-center justify-between gap-2 border-b border-purple-500/30 pb-2.5">
            ...
            <div className="flex flex-wrap items-center gap-2 font-mono text-[11px]">
              <span className="bg-slate-950/80 px-2.5 py-1 rounded-lg border border-slate-800 text-amber-300 font-bold">
                🔥 NASA Active Fires: {targetSummary?.active_fires_count || 0} ({targetSummary?.active_frp_mw || 0.0} MW FRP)
              </span>
              <span className="bg-slate-950/80 px-2.5 py-1 rounded-lg border border-slate-800 text-blue-300 font-bold">
              ...
              <span className="bg-slate-950/80 px-2.5 py-1 rounded-lg border border-slate-800 text-purple-300 font-bold">
                🛡️ BLH Inversion: {targetSummary?.satellite_telemetry?.blh_m || 650}m
              </span>
            </div>
          </div>

          <p className="text-xs text-slate-200 font-medium leading-relaxed bg-slate-950/70 p-4 rounded-2xl border border-slate-800/80">
            {simData.satellite_reasoning}
          </p>
          ...
          {targetSummary?.chemical_mass_balance_pct && (
            <div className="grid grid-cols-3 gap-3 pt-1">
              ...
              <div className="bg-slate-950/60 border border-slate-800/80 p-2.5 rounded-xl text-center">
                <span className="text-[10px] text-blue-400 font-bold block">🚗 Vehicular Exhaust Share</span>
          ...
          <div className="glass-panel p-6 rounded-3xl space-y-5 border border-slate-800 bg-slate-900/60">
            <div className="flex items-center justify-between border-b border-slate-800 pb-3">
              <span className="text-xs font-extrabold text-slate-200 uppercase tracking-wider flex items-center gap-2">
                <Sliders size={16} className="text-purple-400" /> Step 1: Select District & Levers
              </span>
            ...
            <div className="glass-panel p-5 rounded-3xl space-y-2.5 border border-purple-500/30 bg-gradient-to-r from-purple-950/20 to-slate-900/40">
              <div className="flex items-center justify-between">
                <span className="text-xs font-black text-purple-300 uppercase tracking-wider flex items-center gap-2">
                  <School size={16} className="text-purple-400" /> {district} District Advisory & Public Health
                </span>
            ...
            <div className="glass-panel p-6 rounded-3xl space-y-4 border border-slate-800 bg-slate-900/60">
              <div className="flex justify-between items-center border-b border-slate-800 pb-3">
                <div>
                  ...
                  <p className="text-[11px] text-slate-400 mt-0.5">Cities that receive cleaner air when {district} cuts emissions:</p>
                </div>
              </div>
                ...
                {simData.downwind_impact.map((item) => (
                  <div key={item.city} className="bg-slate-950/70 border border-slate-800/80 rounded-2xl p-3.5 space-y-2 hover:border-emerald-500/40 transition-all">
                    <div className="flex justify-between items-center">
                      ...
                      <span className="text-[10px] font-mono text-slate-400">{item.distance_km} km ({item.transit_hours}h ETA)</span>
                    </div>
